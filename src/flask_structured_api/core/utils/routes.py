...
def get_filtered_routes(
    include_methods: bool = False, check_auth: bool = True
) -> Dict[str, Any]:
    ...
    for rule in current_app.url_map.iter_rules():
        # Skip system routes
        if (
            any(x in rule.rule for x in ["/openapi", "/static", "/swagger", "/redoc"])
        ...
        elif rule.endpoint not in authenticated_endpoints and hasattr(
            view_func, "_roles"
        ):
            if not hasattr(g, "user") or not has_required_roles(
                g.user, view_func._roles
            ):
                continue
